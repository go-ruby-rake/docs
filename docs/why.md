...
`go-ruby-rake/rake` reimplements Ruby's `rake` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-rake/rake` directly, with no Ruby runtime;
